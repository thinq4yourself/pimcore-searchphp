            ...
            <?php if (is_array($this->availableCategories) and count($this->availableCategories) > 0) { ?>
                ...
                <select id="searchCat" name="cat">
                    ...
                    <?php foreach ($this->availableCategories as $category) { ?>
                        <option <?php if ($this->category == $category) { ?>selected="selected"<?php } ?> value="<?php echo $category ?>">
                            <?php echo $this->translate('search_category_' . $category) ?>
                        </option>
                    <?php } ?>
                </select>
            <?php } ?>
...
<div id="search_info">
    ...
    if (count($this->searchResults) >= 1) {
        ...
        ?>
        <?php echo $this->translate('search_showing_results') ?>
        <?php echo $start + 1 ?> - <?php echo $end ?>
        <?php echo $this->translate('search_results_of') ?>
        <?php echo $this->total ?><br/>
    <?php } else {
...
<?php if (!empty($this->suggestions)) { ?>
    ...
    <?php for ($i = 0; $i < 5; $i++) { ?>
        ...
        <a style="cursor:pointer;" id="search_result_additional_suggestions_hint" onclick="$('#search_result_additional_suggestions_hint').hide();$('#search_result_additional_suggestions').show()">
            <?php echo (count($this->suggestions) - 5) . ' ' . $this->translate('more_search_suggestions') ?>
        </a>
    <?php } ?>
<?php } ?>
        ...
        <?php foreach ($categorizedSearchResults as $key => $categoryResults) { ?>
            ...
            <?php foreach ($categoryResults as $searchResult) { ?>
                    ...
                    echo implode(" ", $searchResult['categories']);
                        ...
                        <a href="<?php echo $searchResult['url'] ?>"><?php if (!empty($searchResult['title'])) {
                    echo $searchResult['title'];
                } else {
